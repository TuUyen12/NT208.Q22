...
const Header = () => {
  ...
  const handleNav = (item) => {
    if (["contact", "services", "zodiac"].includes(item.to)) {
      // Chuyển về Home và scroll tới section nếu có
      navigate("/");
      // Do Home mount lại, cần đợi rồi scroll
      setTimeout(() => {
    ...
    >
      ...
      >
        ...
        >
          {navItems.map((item) => {
            const isActive =
              item.activePath && location.pathname === item.activePath;
            return (
              <button
                key={item.label}
                className="nav-link"
